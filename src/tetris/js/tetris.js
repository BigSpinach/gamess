var tetris={
	...
	isGameOver:function(){
		//遍历nextShape中每个Dfgd
		for(var i=0;i<this.nextShape.Dfgds.length;i++){
			var Dfgd=this.nextShape.Dfgds[i];
		//	如果wall中和Dfgd相同的位置不等于null
			if(this.wall[Dfgd.r][Dfgd.c]!=null){
				return true
			}
		}
		return false;
	...
	paintNextShape:function(){//专门负责绘制nextShape图形
		...
		for(var i=0;i<this.nextShape.Dfgds.length;i++){
			...
			img.src=Dfgd.img;
			img.style.top=
			(Dfgd.r+1)*this.Dfgd_SIZE+this.OFFSET+"px";
			img.style.left=
			(Dfgd.c+11)*this.Dfgd_SIZE+this.OFFSET+"px";
	...
	canRotate:function(){//能否顺时针旋转
		//遍历shape中每个Dfgd
		for(var i=0;i<this.shape.Dfgds.length;i++){
			var Dfgd=this.shape.Dfgds[i];
		//	如果Dfgd的c<0或c>=CN或r>=RN或在wall中相同位置有格
			if(Dfgd.c<0||Dfgd.c>=this.CN
				||Dfgd.r<0||Dfgd.r>=this.RN
				||this.wall[Dfgd.r][Dfgd.c]!=null){
				return false;
			}
		}
		return true;
	...
	canLeft:function(){
		...
		for(var i=0;i<this.shape.Dfgds.length;i++){
			var Dfgd=this.shape.Dfgds[i];
		//	如果Dfgd的c==0或wall中当前Dfgd左侧位置不等于null
			if(Dfgd.c==0
				||this.wall[Dfgd.r][Dfgd.c-1]!=null){
				return false;
			}
		}//(遍历结束)
		return true;
	...
	canDown:function(){//专门检查是否可以下落
		//遍历shape中的Dfgds数组中每个格
		for(var i=0;i<this.shape.Dfgds.length;i++){
			var Dfgd=this.shape.Dfgds[i];
		//	如果当前Dfgd的r等于RN-1，或在wall数组中和当前Dfgd位置对应的下一行的元素不等于null
			if(Dfgd.r==this.RN-1
				||this.wall[Dfgd.r+1][Dfgd.c]){
	...
	moveDown:function(){//负责主角图形shape下落一步
		...
		//	才调用shape图形的moveDown方法()
			this.shape.moveDown();
		}else{//将shape中每个Dfgd放入wall的相同位置
			//遍历shape中的每个Dfgd对象
			for(var i=0;i<this.shape.Dfgds.length;i++){
			//	将shape中当前Dfgd临时保存在变量Dfgd中
				var Dfgd=this.shape.Dfgds[i];
			//	将当前Dfgd对象放到wall数组中相同r行c列的位置上
				this.wall[Dfgd.r][Dfgd.c]=Dfgd;
			}
	...
	paintWall:function(){//绘制墙的方法
		...
		for(var r=0;r<this.RN;r++){
			for(var c=0;c<this.CN;c++){
		//	将wall中当前元素临时保存在变量Dfgd中
				var Dfgd=this.wall[r][c];
				if(Dfgd){//	如果Dfgd有效
		//		实例化一个Image对象，保存在变量img中
					var img=new Image();
		//		img的src等于Dfgd的img
					img.src=Dfgd.img;
		//		img的top等于Dfgd的r*Dfgd_SIZE+OFFSET
					img.style.top=
						Dfgd.r*this.Dfgd_SIZE+this.OFFSET+"px";
		//		img的left等于Dfgd的c*Dfgd_SIZE+OFFSET
					img.style.left=
						Dfgd.c*this.Dfgd_SIZE+this.OFFSET+"px";
		//		将img追加到frag中
					frag.appendChild(img);
				}
			}
	...
	paintShape:function(){//专门负责绘制当前shape图形
		...
		//	new一个Image元素，保存在变量img中
			var img=new Image();
		//	将当前Dfgd对象临时保存在变量Dfgd中
			var Dfgd=this.shape.Dfgds[i];
		//  设置img的src属性为当前Dfgd对象的img属性
			img.src=Dfgd.img;
		//  设置img的top属性为当前Dfgd对象的r*Dfgd_SIZE+OFFSET
			img.style.top=
				Dfgd.r*this.Dfgd_SIZE+this.OFFSET+"px";
		//  设置img的left属性为当前Dfgd对象的c*Dfgd_SIZE+OFFSET
			img.style.left=
				Dfgd.c*this.Dfgd_SIZE+this.OFFSET+"px";
		//	将img追加到frag对象中
			frag.appendChild(img);
