...
Shape.prototype={
	...
	moveDown:function(){//this-->shape
		//遍历当前图形对象中的Dfgds数组
		//	将当前图形对象的r++
	for(var i=0;i<this.Dfgds.length;this.Dfgds[i++].r++);
		
	},
	moveLeft:function(){
	for(var i=0;i<this.Dfgds.length;this.Dfgds[i++].c--);
	},
	moveRight:function(){
	for(var i=0;i<this.Dfgds.length;this.Dfgds[i++].c++);
	},
	...
	rotate:function(){
		...
		//获得shape中参照格
		var orgDfgd=this.Dfgds[this.orgi];
		//遍历当前图形中的Dfgds数组(i)
		for(var i=0;i<this.Dfgds.length;i++){
		//将当前格的r设置为参照格r+state中i位置的对象的r
			this.Dfgds[i].r=orgDfgd.r+state[i].r;
		//将当前格的c设置为参照格c+state中i位置的对象的c
			this.Dfgds[i].c=orgDfgd.c+state[i].c;
		}
	}
}
//每种图形类型的对象
function O(){
	...
	this.Dfgds=[
 new Dfgd(0,4,this.IMGS.O),	new Dfgd(0,5,this.IMGS.O),
 new Dfgd(1,4,this.IMGS.O),  new Dfgd(1,5,this.IMGS.O)
	];
	this.states=[
...
function T(){
	...
	this.Dfgds=[
		new Dfgd(0,3,this.IMGS.T),	
		new Dfgd(0,4,this.IMGS.T),
		new Dfgd(0,5,this.IMGS.T),  
		new Dfgd(1,4,this.IMGS.T)
	];
	this.states=[
...
function I(){
	Shape.call(this,1);//借用父类型构造函数
	this.Dfgds=[
		new Dfgd(0,3,this.IMGS.I),	
		new Dfgd(0,4,this.IMGS.I),
		new Dfgd(0,5,this.IMGS.I),  
		new Dfgd(0,6,this.IMGS.I)
	];
	this.states=[
